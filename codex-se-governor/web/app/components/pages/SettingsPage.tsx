...
export function SettingsPage({ language, workspace, project, navigate, localRunner, onLocalRunner, onPolicyChange, onClearWorkspace, theme, onTheme }: WorkspacePageProps & {
  ...
  return <div className="settings-workspace">
    ...
    <div className="settings-layout">
      <nav className="settings-nav" aria-label={text(language, "设置分类", "Settings sections")}>{sections.map(([id, label]) => <button key={id} className={section === id ? "active" : ""} onClick={() => setSection(id)}>{label}</button>)}</nav>
      <section className="settings-content">
        {section === "general" && <>
          ...
          <div className="setting-row"><div><b>{text(language, "主题", "Theme")}</b><p>{text(language, "系统模式会跟随设备外观。", "System mode follows the device appearance.")}</p></div><div className="segmented-control"><button className={theme === "system" ? "active" : ""} onClick={() => onTheme("system")}>{text(language, "系统", "System")}</button><button className={theme === "light" ? "active" : ""} onClick={() => onTheme("light")}>{text(language, "浅色", "Light")}</button><button className={theme === "dark" ? "active" : ""} onClick={() => onTheme("dark")}>{text(language, "深色", "Dark")}</button></div></div>
