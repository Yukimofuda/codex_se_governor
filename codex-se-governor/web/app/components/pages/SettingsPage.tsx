...
export function SettingsPage({ language, workspace, project, navigate, localRunner, onLocalRunner, onPolicyChange, onClearWorkspace, theme, onTheme }: WorkspacePageProps & {
  ...
  const sections: Array<[SettingsSection, string]> = [
    ...
    ["providers", text(language, "需求 AI", "Requirement AI")],
    ["runner", text(language, "本机 Codex", "Local Codex")],
  ...
  return <div className="settings-workspace">
    <PageHeader eyebrow={text(language, "工作区", "Workspace")} title={text(language, "设置", "Settings")} />
    <div className="settings-layout">
      ...
      <section className="settings-content">
        {section === "general" && <>
          <div className="section-heading"><div><h2>{text(language, "界面与本地项目", "Interface and local projects")}</h2></div><Settings /></div>
        ...
        {section === "providers" && <>
          <div className="section-heading"><div><h2>{text(language, "需求整理服务", "Requirement AI provider")}</h2></div>{providerStatus?.provider?.status === "connected" ? <span className="connection-status connected"><Check />{text(language, "连接正常", "Connected")}</span> : providerStatus?.configured ? <span className="connection-status configured">{text(language, "已保存，待测试", "Saved, not tested")}</span> : <span className="connection-status">{text(language, "未配置", "Not configured")}</span>}</div>
        ...
        {section === "policy" && (project ? <>
          <div className="section-heading"><div><h2>{text(language, "发布门禁", "Release gates")}</h2></div></div>
        ...
        {section === "limits" && <>
          <div className="section-heading"><div><h2>{text(language, "哪些操作在哪里发生", "Where each operation happens")}</h2></div></div>
        ...
        {section === "security" && <>
          <div className="section-heading"><div><h2>{text(language, "数据与密钥", "Data and credentials")}</h2></div><ShieldCheck /></div>
