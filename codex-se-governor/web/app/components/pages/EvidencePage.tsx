...
function artifactSource(source: EngineeringArtifact["source"]): EvidenceSource {
  ...
  if (source === "human" || source === "ai-assisted") return "attested";
  return "unknown";
}
...
function ArtifactEditor({ language, artifact, readOnly, onSave }: { language: WorkspacePageProps["language"]; artifact: EngineeringArtifact; readOnly: boolean; onSave: (artifact: EngineeringArtifact, confirm?: boolean) => void }) {
  ...
  return <section className="artifact-inspector">
    <header><div><span className="section-label">{stageText(language, { key: artifact.stageKey, label: artifact.stageKey })}</span><h2>{artifact.fileName}</h2><p>{artifact.status === "draft" ? text(language, "系统已根据项目与需求生成草稿，确认前不会计为完成证据。", "This draft comes from project and requirement data and does not count as completed evidence until confirmed.") : text(language, "此版本已作为当前工程流程的确认记录。", "This version is recorded as confirmed evidence for the current workflow.")}</p></div><div className="badge-row"><SourceBadge source={artifactSource(artifact.source)} /><StatusBadge status={artifact.status === "draft" ? "pending" : "passed"} label={artifact.status === "verified" ? text(language, "已验证", "Verified") : artifact.status === "confirmed" ? text(language, "已确认", "Confirmed") : text(language, "草稿", "Draft")} /></div></header>
