...
export function ReleasePage({ language, workspace, project, navigate, onGenerate, onApprove, onOpenStage }: WorkspacePageProps & { onGenerate: (run: WorkflowRun) => void; onApprove: (run: WorkflowRun) => void; onOpenStage: (runId: string, stage: WorkflowStage) => void }) {
  ...
  const rows: Array<{ label: string; value: string; status: WorkStatus; action: string; target: "requirements" | "evidence" | "checks" | "run"; stageKey?: "security" }> = [
    ...
    { label: text(language, "安全审查", "Security review"), value: securityStatus === "passed" ? text(language, "通过", "Passed") : text(language, "未完成", "Incomplete"), status: securityStatus, action: text(language, "查看安全阶段", "Open security stage"), target: "run", stageKey: "security" },
  ...
  const verdict = manifest ? text(language, ({ ready: "可发布", conditional: "有条件通过", blocked: "已阻断", unknown: "未知" } as const)[manifest.status], ({ ready: "READY", conditional: "CONDITIONAL PASS", blocked: "BLOCKED", unknown: "UNKNOWN" } as const)[manifest.status]) : text(language, "尚未计算", "NOT CALCULATED");
  const policyName = policy.id === "strict" ? text(language, "严格策略", "Strict policy") : policy.id === "standard" ? text(language, "标准策略", "Standard policy") : text(language, "自定义策略", "Custom policy");
  ...
  return <div className="release-workspace">
    ...
    <div className="release-layout">
      <section className="release-gates">
        <header><div><span className="section-label">{text(language, "强制门禁", "Required gates")}</span><h2>{text(language, "发布条件", "Release conditions")}</h2></div><span className="policy-badge">{policyName}</span></header>
        <div>{rows.map((row) => <article key={row.label}><div><b>{row.label}</b><small>{row.value}</small></div><StatusBadge status={row.status} language={language} /><button className="text-button" onClick={() => { const stage = row.stageKey ? run.stages.find((item) => item.key === row.stageKey) : undefined; if (stage) onOpenStage(run.id, stage); else navigate(row.target); }}>{row.action}</button></article>)}</div>
      </section>
      <section className="release-findings">
        <header><div><span className="section-label">{text(language, "判断依据", "Decision basis")}</span><h2>{text(language, "阻断与警告", "Blockers and warnings")}</h2></div></header>
        {!manifest ? <p className="muted">{text(language, "计算后将列出每个未满足条件及其来源。", "Calculate readiness to list every unmet condition and its source.")}</p> : <>
          ...
          <footer><span>{manifest.checkIds.length} {text(language, "项检查", "checks")}</span><span>{manifest.evidenceIds.length} {text(language, "项证据", "evidence items")}</span><span>{new Date(manifest.generatedAt).toLocaleString(language === "zh" ? "zh-CN" : "en-US")}</span></footer>
        </>}
      </section>
    </div>
  </div>;
