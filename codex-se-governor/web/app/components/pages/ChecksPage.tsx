...
export function ChecksPage({ language, workspace, project, navigate, onImportManifest, onSaveAdoption }: WorkspacePageProps & {
  ...
  return <div>
    <PageHeader eyebrow={`${project.name} · ${text(language, "检查", "Checks")}`} title={text(language, "检查结果", "Checks")} description={text(language, "机器检查、AI 审查和人工决定使用不同标记，避免把判断来源混在一起。", "Machine checks, AI reviews, and human decisions use distinct labels so their sources remain clear.")} actions={<div className="segmented-control"><button className={mode === "results" ? "active" : ""} onClick={() => setMode("results")}>{text(language, "运行结果", "Run results")}</button><button className={mode === "repository" ? "active" : ""} onClick={() => setMode("repository")}>{text(language, "仓库接入检查", "Repository adoption")}</button></div>} />
    {mode === "results" ? <>
      {!run ? <EmptyState icon={CheckSquare2} title={text(language, "还没有运行", "No run yet")} description={text(language, "开始工作流后，可以在这里导入或查看检查结果。", "Start a run to import and inspect validation results.")} actions={<button className="primary-button" onClick={() => navigate("plan")}>{text(language, "前往计划", "Open plan")}</button>} /> : <div className="checks-layout">
        <section className="workspace-panel checks-list">
          ...
          {checks.length ? <ul>{checks.map((check) => <li key={check.id}><button className={selected?.id === check.id ? "selected" : ""} onClick={() => setSelectedId(check.id)}><span><b>{recordText(language, check.label)}</b><small>{check.category}</small></span><StatusBadge status={check.status} /></button></li>)}</ul> : <div className="inline-empty"><Info /><span>{text(language, "此运行还没有检查结果。", "This run has no check results yet.")}</span></div>}
      ...
      <section className="workspace-panel repository-input">
        ...
        <div className="segmented-control wide"><button className={repositoryMode === "zip" ? "active" : ""} onClick={() => setRepositoryMode("zip")}>ZIP</button><button className={repositoryMode === "github" ? "active" : ""} onClick={() => setRepositoryMode("github")}>GitHub</button><button className={repositoryMode === "paths" ? "active" : ""} onClick={() => setRepositoryMode("paths")}>{text(language, "文件清单", "File list")}</button></div>
