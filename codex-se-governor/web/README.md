...
- **概览**：项目需求列表、最近运行，以及所选需求的检查和发布记录。
- **项目**：创建项目或打开集中管理的录制示例。
- **需求**：选择任意已有需求继续编辑，或创建新需求。未确认草稿也会自动保存；侧栏显示实际保存状态，失败时可重试。
...
- 验证清单显示为“导入结果”，保留原始记录；当前未独立验证来源签名，不能仅凭导入成功认定测试真实执行。
...
临时令牌从应用内 **设置 → 本机 Codex** 生成。分析任务使用 Codex `read-only` 沙箱，实现任务使用 `workspace-write`；Runner 不启用 `danger-full-access`，不通过 shell 拼接命令，也不监听局域网地址。详见 `docs/LOCAL_CODEX_RUNNER.md`。
...
- 390px 移动端、820px 平板、桌面深色及英文布局与横向溢出检查；
- 多需求切换、未确认草稿刷新恢复，以及保存失败后的重试。

## 本轮产品审查

视觉研究、验收目标及产品缺口见 [工作台产品审查](docs/PRODUCT_REVIEW_WORKSPACE.md)。本轮修复工作上下文和草稿连续性，不代表完整无人值守执行已达可交付标准。

当前本机 Runner 尚未绑定项目仓库指纹；批准计划与实际执行输入仍需不可变版本绑定；导入结果没有来源签名，不能自动满足已验证证据门禁。请勿将当前发布记录当作独立安全认证。
