...
- `v0.7.2`: isolated/tiered validation, one-pass result artifacts, exact patch/source archives, release SHA manifest, and course provenance evidence.
...
- Immutable release archives include the full patch version. A major/minor alias may exist only as a convenience copy.
...
- For v0.7.2 and later, use `run_full_validation.py --standard` for PRs and `--release` for tags; distribute only script-generated exact release/source archives.
